...
CanALE_space <- CanALE_space %>% 
  mutate(active_trans = sum(Walked, Bicycle)) %>% 
  mutate(active_trans_pct = (active_trans/Pop)) %>% 
...
bivariate_quantiles <- CanALE_space %>%
  mutate(mean_quantiles_variable_one = cut(Imm_pct,
                               ...
                               labels = labels,
                               include.lowest = T)) %>%
  mutate(mean_quantiles_variable_two = cut(active_trans_pct,
...
legend <- ggplot() +
  ...
  scale_fill_identity() +
  labs(x = "More active transit commuters ->",
...
map_total <-   ggplot(
  ...
  labs(x = NULL,
         y = NULL,
         subtitle = "Montreal's immigrant population and those who walked or biked to work",
         title = paste0("Percent of Immigrants",
                           " vs. Percent of Active Transit Commuters, 2016"),
         caption = "Fig. 1. Percentage of immigrants vs. Percent of Active Transit Commuters (Biked + Walked)"
       ) +
  # add the theme
  theme_map() 
...
par(bg=NA)
library(png)
png(filename = "figures/immigrant vs active transit.png", width = 8, height = 8, units = "in", res=1000, bg = "transparent")
ggdraw() +
  draw_plot(map_total, 0, 0, 1.05, 1.05) +
  draw_plot(legend, 0.015, 0.0, 0.35, 0.35) 

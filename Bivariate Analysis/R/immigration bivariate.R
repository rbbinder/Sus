...
names(CanALE_space)
plot(CanALE_space[64])
plot(CanALE_space[65])
...
bivariate_color_scale <- tibble(
  "3 - 3" = "#3F2949", # high immigrant, high transit ALE
  "2 - 3" = "#435786",
  "1 - 3" = "#4885C1", # low immigrant, high transit ALE
  "3 - 2" = "#77324C",
  "2 - 2" = "#806A8A", # medium immigrant, medium transit ALE
  "1 - 2" = "#89A1C8",
  "3 - 1" = "#AE3A4E", # high immigrant, low transit ALE
  "2 - 1" = "#BC7C8F",
  "1 - 1" = "#CABED0" # low immigrant, low transit ALE
) %>%
  gather("group", "fill")
...
legend <- ggplot() +
  ...
  scale_fill_identity() +
  labs(x = "Higher Transit Active Living Environment ->",
...
labs(x = NULL,
     y = NULL#,
     # subtitle = "Montreal's active living and immigration",
     # title = paste0("Percent of population that are immigrants",
                      # " vs. can-ALE transit Active Living Environment score, 2016"),
     # caption = "Fig. 1.Immigrant population vs. Transit ALE score
) +
  # add the theme
  theme_map() 
...
map_total <- ggplot(data = cropped_roads) # use the same dataset as before
   +
  scale_alpha(name = "",
              range = c(0.6, 0),
              guide = F) + # suppress legend
